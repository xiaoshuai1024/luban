...
describe('user API', () => {
  ...
  it('getUsers calls request.get with /users', async () => {
    vi.mocked(request.get).mockResolvedValue({ data: { list: [], total: 0 } } as any);
    await getUsers();
  ...
  it('getUsers with params passes them', async () => {
    ...
    expect(request.get).toHaveBeenCalledWith('/users', { params: { page: 1, size: 10, keyword: 'test' } });
  });

  it('getUser calls GET /users/:id', async () => {
    vi.mocked(request.get).mockResolvedValue({ data: { id: 'u1' } } as any);
    await getUser('u1');
    expect(request.get).toHaveBeenCalledWith('/users/u1');
  });

  it('createUser calls POST /users', async () => {
    vi.mocked(request.post).mockResolvedValue({ data: { id: 'new' } } as any);
    await createUser({ username: 'newuser', password: 'pass', name: 'New', role: 'editor' });
    expect(request.post).toHaveBeenCalledWith('/users', { username: 'newuser', password: 'pass', name: 'New', role: 'editor' });
  });
  ...
  it('setUserStatus calls PATCH /users/:id/status', async () => {
    vi.mocked(request.patch).mockResolvedValue({ data: {} } as any);
